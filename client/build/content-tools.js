...
(function() {
  var AttributeUI, ContentTools, CropMarksUI, StyleUI, TimeDialog, exports, _EditorApp,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __slice = [].slice;
  ...
  ContentTools = {
    ...
    DEFAULT_TOOLS: [['bold', 'italic', 'link', 'align-left', 'align-center', 'align-right', 'text-color', 'background-color', 'superscript', 'subscript'], ['heading', 'subheading', 'paragraph', 'unordered-list', 'ordered-list', 'table', 'indent', 'unindent', 'line-break'], ['image'], ['undo', 'redo', 'remove']],
...
// --------------------------------------------------------------------------
 ...
 ContentTools.Tools.ColorTool = (function(_super) {
  ...
  function ColorTool() {
   ...
   ContentTools.ToolShelf.stow(ColorTool, 'text-color');
   

  ColorTool.label = 'Text Color';

  ColorTool.icon = 'text-color';

  ColorTool.tagName = 'span';

  ColorTool.apply = function (element, selection, callback) {
    var app, dialog, modal, from, selectTag, to, _ref, rect, measureSpan, domElement;
    ...
    dialog = new ColorDialog(ColorTool.getColorValue(element, selection));
    ...
    dialog.addEventListener('save', (function(_this) {
      return function (ev) {
        var colorValue;
        colorValue = ev.detail().style;
        element.content = element.content.unformat(from, to, 'span');

        if (colorValue) {
              color = new HTMLString.Tag('span', { style:`color:${colorValue}`});
              element.content = element.content.format(from, to, color);
              element.updateInnerHTML();
              element.taint();
              modal.unmount();
              dialog.hide();
  ...
  ColorTool.getColorValue = function(element, selection) {
    var c, from, selectedContent, tag, to, _i, _j, _len, _len1, _ref, _ref1, _ref2;
  ...
  ColorDialog = (function(_super) {
    ...
    ColorDialog.prototype.mount = function() {
      ...
      this._domInput.setAttribute('placeholder', '');
...
// // ********* Function below will have icon focused when the texts are selected
//     ColorTool.canApply = function(element, selection) {
//     // if (!element.content) {
//       //   return false;
//       // } else {
//       //   return selection && !selection.isCollapsed();
...
// //*****Function below will have icon pressed. This means color icon will highlighted
//     ColorTool.isApplied = function(element, selection) {
//       var from, to, _ref;
//       if (element.content === void 0 || !element.content.length()) {
//         return false;
//       }
...
//   ColorTool.apply = function (element, selection, callback) {
//     var app, dialog, modal, from, to, _ref, rect, measureSpan, domElement, selectTag, cursor, tail, tip, toolDetail, nr;
...
//     // The plus 1 is whatever character ahead of the cursor
//     //This is causing the curser to jump back to the first line and increment by one
//     cursor = selection.get()[0] + 1;


//       //this is where we will add the span element with the attribute of class="ct--puesdo-select"
//       tip = element.content.substring(0, from);
//       tail = element.content.substring(to);
//       console.log("TIP & TAIL----->", tip, tail)

      
//       // br = new HTMLString.String('<span></span>', element.content.preserveWhitespace());
//       br = new HTMLString.String('<br>', element.content.preserveWhitespace());
//       element.content = tip.concat(br, tail);
...
//       // This makes the cursor stays in place when element is inserted
...
//     //modal to close the dialog as soon as user clicks away from dialog
//     modal = new ContentTools.ModalUI(transparent = true, allowScrolling = true);
...
//     // placing the dialog on the focused content
//     domElement = element.domElement();
//     measureSpan = domElement?.getElementsByClassName('ct--puesdo-select');
//     rect = measureSpan[0]?.getBoundingClientRect();
...
//     //This is where we add style attribute with its value if parent dont have a style attribute or remove it if the parent has one
//     dialog.addEventListener('save', (function (_this) {
...
//   })(ContentTools.LinkDialog);
 ...
 //-------------------------------------Background Color-----------------------------------
 ContentTools.Tools.BackgroundColorTool = (function(_super) {

  __extends(BackgroundColorTool, _super);

  function BackgroundColorTool() {
    return BackgroundColorTool.__super__.constructor.apply(this, arguments);
  }

   ContentTools.ToolShelf.stow(BackgroundColorTool, 'background-color');
   
  BackgroundColorTool.label = 'Background Color';
  BackgroundColorTool.icon = 'background-color';
   
   
   BackgroundColorTool.canApply = function (element, selection) {
     
    return element?.content !== void 0;
  };
   
  BackgroundColorTool.isApplied = function(element, selection) {
    
    if (!this.canApply(element)) {
      return false;
    }

    return element.domElement().getAttribute("style") !== null;
  };
  ...
  BackgroundColorTool.apply = function (element, selection, callback) {
    var app, dialog, modal, rect, domElement;
    ...
    //placing the dialog   
    dialog = new BackgroundColorDialog(BackgroundColorTool.getColorValue(element, selection));
    dialog.position([rect.left + (rect.width / 2) + window.scrollX, rect.top + (rect.height / 2) + window.scrollY]);
    ...
    dialog.addEventListener('save', (function(_this) {
      return function (ev) {
        ...
        backgroundColorValue = ev.detail().style;
        domElement = element.domElement().removeAttribute("style");

        if (backgroundColorValue) {
          domElement = element.attr("style", `background-color:${backgroundColorValue}`); 
          // console.log(element)
          element.updateInnerHTML();
          // ***** Perhaps this line below is now letting the changes to save or update as user click green button
          element.taint();
          modal.unmount();
          dialog.hide();
          element.updateInnerHTML();
   ...
   BackgroundColorTool.getColorValue = function (element, selection) { 
     console.log(element.domElement().getAttribute("style"))
     
     if (element.domElement().getAttribute("style") !== null) {
      console.log(element.domElement().getAttribute("style"))
       
      return element.domElement().getAttribute("style")
    }
  };
  ...
  BackgroundColorDialog = (function(_super) {
    __extends(BackgroundColorDialog, _super);

    function BackgroundColorDialog() {
      return BackgroundColorDialog.__super__.constructor.apply(this, arguments);
    }

    BackgroundColorDialog.prototype.mount = function() {
      ColorDialog.__super__.mount.call(this);
      this._domInput.setAttribute('type', 'color');
      this._domInput.setAttribute('name', 'color');
      this._domInput.setAttribute('placeholder', '');
    ...
    BackgroundColorDialog.prototype.save = function() {
      var detail;
      detail = {
        style: this._domInput.value.trim()
      };
 ...
 ContentTools.Tools.Superscript = (function(_super) {
  ...
  Superscript.canApply = function(element, selection) {
    if (!element.content) {
      return false;
    }
    return selection && !selection.isCollapsed();
  ...
  Superscript.isApplied = function(element, selection) {
    var from, to, _ref;
    if (element.content === void 0 || !element.content.length()) {
      return false;
    }
  ...
  Superscript.apply = function(element, selection, callback) {
    var from, to, toolDetail, _ref;
    toolDetail = {
      'tool': this,
      'element': element,
 ...
 ContentTools.Tools.Subscript = (function(_super) {
  ...
  Subscript.canApply = function(element, selection) {
    if (!element.content) {
      return false;
    }
    return selection && !selection.isCollapsed();
  ...
  Subscript.isApplied = function(element, selection) {
    var from, to, _ref;
    if (element.content === void 0 || !element.content.length()) {
      return false;
    }
  ...
  Subscript.apply = function(element, selection, callback) {
    var from, to, toolDetail, _ref;
    toolDetail = {
      'tool': this,
      'element': element,
